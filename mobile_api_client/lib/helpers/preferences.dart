...
class Preference {
  ...
  static Future<JSON?> getUserAccessToken() async {
    final SharedPreferences prefs = await _prefs;
    final String? tokenString = prefs.getString(ApiEndpoint.tokenKey);
  ...
  static Future<void> setUserAccessToken({JSON? token}) async {
    final SharedPreferences prefs = await _prefs;
    if (token == null) {
      await prefs.setString(ApiEndpoint.tokenKey, '');
    }

    final String tokenString = json.encode(token);
    await prefs.setString(ApiEndpoint.tokenKey, tokenString);
  }
}
